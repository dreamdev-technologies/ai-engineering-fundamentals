...
5. Start a fresh session (`/clear` in Claude Code, New Chat in Copilot) before each story so failed approaches from the last story do not pollute the next. Repeat for story two, then story three. When a story is ambiguous, do not guess silently and do not let the agent guess: decide as a pair, then encode the decision as a test. The test is where a decision lives; the prompt is not.
