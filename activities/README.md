...
| 03 | Config check-up | People | 25 min | A repo you work in daily, in the tool you normally use on it; the exercise repo as fallback |
| 04 | The LLM wiki: start one for your own system | People | 30 min | A repo you work in daily, in the tool you normally use on it; the exercise repo as fallback |
| 05 | Acceptance tests first | People | 45 min | 03; 04 helps |
| 06 | Goal-driven delegation | People | 30 min | 05 (its verification) |
| 07 | Characterisation tests | People | 40 min | The exercise repo |
| 08 | Agent creation | People | 35 min | 05 and 07 (the work it bottles) |
| 09 | Strangler extraction | People | 40 min | 07 (its tests); 08 if it ran |
...
08 sits where it does for two reasons. It asks people to specify an agent for a job, which only goes well once they have done that job by hand in 05 and 07 and know what the agent has to be forbidden from doing. And it is followed immediately by 09, which puts the agents on new work, so a loose specification shows up as a problem the same morning rather than never.
...
The library is more than one session's worth on purpose. A session opens with the talk and the value stream map, then activities are chosen one at a time from what the room says it needs, and it ends with the close. Whatever does not run is there for the next session; each person's copy of the repository carries the state between them. The dependencies in the table are the only constraint on the order.
...
Activities 05 to 09 use the exercise repository and touch no other codebase. Activities 03 and 04 are the exceptions by design: they run in a repository you already work in, using whichever tool you normally use on it, and change no code there. 03 prunes the instruction files; 04 starts a wiki on a branch. People work in Claude Code or GitHub Copilot; the instructions call out any difference.
