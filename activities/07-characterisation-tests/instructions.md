...
Two things make agents unusually dangerous on this kind of code. The first is that they are built to write the tests a healthy codebase should have: hand a test-generation tool a method that rounds sterling by truncation and it will write a test that asserts proper rounding, watch it fail, and offer to fix the code. The dry run of this activity did exactly that. On legacy code the surprising behaviour is often the behaviour someone downstream depends on, and a test that asserts the "right" answer deletes it silently. The second is scope. Asked to change one thing in an old file, an agent will often restructure the file, and Anthropic's own prompting guides for the Claude 5 models say so: Opus 5 "can also expand the scope of a task, adding steps that weren't requested", and the Fable 5.1 guide has sections on keeping changes to what the task asks for and preferring targeted edits over whole-file rewrites. On a system whose quirks are load-bearing, a rewrite that keeps the tests green but reorganises everything around them is a change nobody can review, and a change nobody can review is a change nobody can trust.
