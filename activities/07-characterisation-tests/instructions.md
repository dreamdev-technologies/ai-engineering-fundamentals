...
Pairs. 40 minutes. The exercise repository: `src/Legacy`, `src/Legacy.Tests`, `wiki/gotchas.md`. How to talk to the tools and use a skill is in [tools.md](../tools.md).
...
5. For each pinned quirk, decide between you: load-bearing or bug? Would anything downstream break if this behaviour changed? Mark the decision with a comment on the test: KEEP with the reason, or BUG with the intended behaviour.
